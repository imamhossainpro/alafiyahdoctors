...
export default function PermissionMatrix({
  ...
  // Calculate overrides vs role (with dependencies applied)
  const calculateOverrides = () => {
    ...
    // ✅ CRITICAL: Apply dependencies to overrides
    return applyDependencies(overrides);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.log('💾 [PermissionMatrix] Saving:', {
        userId: user.id,
        role,
        isActive,
      ...
      try {
        if (roleChanged) {
          await logActivity({
            ...
            recordId: user.id,
            description: `${user.name} এর Role পরিবর্তন: ${getRoleLabel(
              user.role
            )} → ${getRoleLabel(role)}`,
        ...
        if (activeChanged) {
          await logActivity({
            ...
            action: LOG_ACTIONS.UPDATE,
            recordId: user.id,
            description: `${user.name} এর status: ${
        ...
        if (changes.length > 0) {
          await logActivity({
            ...
            oldValue: changes.reduce((acc, c) => {
              acc[c.key] = c.old;
              return acc;
            }, {}),
            newValue: changes.reduce((acc, c) => {
              acc[c.key] = c.new;
              return acc;
            }, {}),
            user: currentUser,
      ...
      console.error('❌ [PermissionMatrix] Save error:', err);
    ...
    >
      ...
      >
        ...
        >
          <div>
            <h3
              style={{ margin: 0, color: '#1e293b', fontSize: '18px' }}
            >
              🔐 User Access Control
            </h3>
            <p
        ...
        >
          ...
          >
            ...
            <input
              type="text"
              placeholder="Permission সার্চ করুন..."
              value={searchPerm}
        ...
        >
          {filteredRegistry.length === 0 ? (
            ...
            filteredRegistry.map((mod) => {
              const allSelected = mod.permissions.every(
                (p) => permissions[p.key]
              );
              const noneSelected = mod.permissions.every(
                (p) => !permissions[p.key]
              );
              return (
                <div
                  ...
                  >
                    ...
                    <div style={{ display: 'flex', gap: '6px' }}>
                      <button
                        ...
                        style={{
                          padding: '4px 12px',
                          background: allSelected ? '#22c55e' : '#f1f5f9',
                      ...
                      <button
                        ...
                        style={{
                          padding: '4px 12px',
                          background: noneSelected ? '#ef4444' : '#f1f5f9',
        ...
        >
          ...
          <div style={{ display: 'flex', gap: '8px' }}>
            ...
            <button
              onClick={handleSave}
              disabled={
                saving || (!isAdmin && !can('user.permission_manage'))
              }
              style={{
                padding: '10px 24px',
                ...
                gap: '6px',
                opacity:
                  !isAdmin && !can('user.permission_manage') ? 0.5 : 1,
              }}
            >
              <Save size={16} />{' '}
              {saving ? 'সংরক্ষণ হচ্ছে...' : 'Save Permissions'}
            </button>
          </div>
        </div>
      </div>
