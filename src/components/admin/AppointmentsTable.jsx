...
import { CheckCircle, XCircle, UserCheck, Archive, Trash2, Clock, Stethoscope, LayoutList } from 'lucide-react';
...
export default function AppointmentsTable({ appointments, onStatusChange, onArchive, isArchivedView }) {
  ...
  const [viewMode, setViewMode] = useState('list'); // 'list' বা 'doctor'
  ...
  // Action Buttons for a single appointment
          ...
          <div style={{ display: 'flex', gap: '5px', background: '#f1f5f9', padding: '4px', borderRadius: '8px' }}>
            <button
              ...
              style={{
                ...
                color: viewMode === 'list' ? '#fff' : '#475569', fontWeight: '600', fontSize: '13px',
                display: 'flex', alignItems: 'center', gap: '5px'
              }}
            ><LayoutList size={14} /> সাধারণ লিস্ট</button>
            <button
              onClick={() => setViewMode('doctor')}
              style={{
                padding: '6px 12px', borderRadius: '6px', border: 'none', cursor: 'pointer',
                background: viewMode === 'doctor' ? '#1c5fa8' : 'transparent',
                color: viewMode === 'doctor' ? '#fff' : '#475569', fontWeight: '600', fontSize: '13px',
                display: 'flex', alignItems: 'center', gap: '5px'
              }}
            ><Stethoscope size={14} /> ডাক্তার ওয়াইজ</button>
          </div>
        )}
      </div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: '1000px' }}>
          ...
          <tbody>
            {appointments.length === 0 && (
              <tr><td colSpan="9" style={{ padding: '20px', textAlign: 'center', color: '#64748b' }}>কোনো বুকিং পাওয়া যায়নি</td></tr>
            )}
              ...
              <div key={doctorName} style={{ marginBottom: '20px', border: '1px solid #e2e8f0', borderRadius: '8px', overflow: 'hidden' }}>
                <div style={{ background: '#f8fafc', padding: '12px 15px', borderBottom: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div>
                    ...
                    <span style={{ color: '#64748b', fontSize: '13px', marginLeft: '10px' }}>{info.dept}</span>
                  </div>
                  ...
                  <tbody>
                    {info.patients.map(appt => (
                      <tr key={appt.id} style={{ borderBottom: '1px solid #eee', fontSize: '14px' }}>
                        ...
                        <td style={{ padding: '10px 12px' }}>{appt.bookingDate}</td>
